...
반응형 웹 개발(mobile, tablet, laptop, desktop)을 적용한 마크업과 인터랙티브 요소를 구현한 프로젝트입니다.
...
스크린 크기별 scss 코드를 작성하면서, 스크린 크기와 상관없이 공통되는 코드영역과 스크린 크기 각각을 위한 코드영역을 구분하는 방식이 맞는지, @media 범위 별로 처음부터 코드를 작성하는 방식이 맞는지 판단할 필요성을 느꼈습니다.  
...
구체적인 디자인 가이드 없이 클론 구현을 진행하며, 개발자 도구로 원본 페이지의 요소들과 스타일 속성 값을 확인하는 작업이 많았습니다. 컴파일/빌드가 진행된 코드이기에 원본 페이지의 요소들 각각에 적용된 여러 클래스의 속성들을 추적하는 것에 제한이 있었고, 적용되는 모든 스타일값을 추적하는 것이 비효율적이라 판단, 레이아웃의 큰 틀을 잡는 데는 참고하되, 세부사항은 최대한 원본 페이지를 따라가는 방식으로 구현했습니다.

페이지를 구성하는 컴포넌트가 늘어나고 코드의 규모와 복잡성이 커질 때 가장 문제가 되는 것이 유지보수임을 느낄 수 있던 프로젝트입니다. 프로토타입 단계에서 보여지는 요소 구현에만 신경 쓸 것이 아니라, 충분한 시간을 들여 당장 보여지지 않는 예상 가능한 문제들을 줄여가는 방식의 코드를 작성해야 합니다.
